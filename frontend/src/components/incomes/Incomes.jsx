...
function Incomes() {
    ...
    const filteredIncomes = incomes
        .filter(income => activeFilter === 'all' || income.category === activeFilter)
        .sort((a, b) => {
            switch (sortBy) {
    ...
    return (
        <div className="p-4 md:p-8 overflow-auto bg-white :bg-gray-800 min-h-screen">
            <InnerLayout>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 bg blue ">
                    <h1 className="text-2xl md:text-3xl font-bold text-white-7">Income Management</h1>
                ...
                <div className="bg-white rounded-xl shadow-md p-6 mb-6 border border-gray-200 flex justify-center items-center">
                    <h2 className="text-xl md:text-2xl font-semibold text-gray-800">
                        Total Income: 
                        <span className="ml-3 text-2xl md:text-3xl font-bold text-green-800">
                            DT {totalIncome()}
                        </span>
                    </h2>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200">
                            <h3 className="text-xl font-semibold mb-4 text-gray-800">Add New Income</h3>
                            <Form type="income" />
                          
                        </div>

                        <div className="lg:hidden mt-6 bg-white rounded-xl shadow-md p-6 border border-gray-200">
                            <h3 className="text-xl font-semibold mb-4 text-gray-800">Income Summary</h3>
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between text-gray-700">
                                    ...
                                    <span className="font-medium">DT {(totalIncome() / (incomes.length || 1)).toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 sticky top-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between text-gray-700">
                                    ...
                                    <span className="font-medium">DT {incomes.length > 0 ? Math.max(...incomes.map(i => i.amount)) : 0}</span>
                                </div>
                                    ...
                                    <div className="space-y-2">
                                        {categories.slice(0, 3).map(cat => (
                                            <div key={cat} className="flex justify-between text-sm text-gray-600">
                                                <span>{cat}</span>
                                                <span>{incomes.filter(i => i.category === cat).length}</span>
                                            </div>
                                        ))}
                                        {categories.length > 3 && (
